...
class PosAppController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return Inertia::render('Apps/Pos-App/Index', [
            'carts' => $carts,
    ...
    public function addToCart(Request $request)
    {
        ...
        if ($cart) {
            ...
            return redirect()->route('apps.pos-app.index')->with('success', 'Product Added Successfully!');
            //dan apabila belum, maka lakukan insert item baru ke dlm cart dgn mengembalikan pesan success
        }
    }

    // Tambahkan method untuk mengedit quantity cart
    public function editCartQuantity(Request $request)
    {
        ...
        // Pastikan quantity yang diinginkan tidak melebihi stok produk
        $product = Product::find($cart->product_id);
        if ($product->stock < $request->qty) {
            return redirect()->back()->with('error', 'Out of Stock Product!');
        }

        // Update quantity dalam cart
        $cart->qty = $request->qty;
        $cart->price = $cart->product->sell_price * $request->qty;
        $cart->save();

        return redirect()->route('apps.pos-app.index')->with('success', 'Cart Quantity Updated Successfully!');
    }
    ...
    public function pendingStore(Request $request)
    {
        ...
        $pendingTransaction = Transaction::create([
            ...
            'invoice' => $invoice,
            'grand_total' => $request->grand_total,
        ]); //kemudian kita insert data transa\ksinya ke dalam databse menggunakan model
        ...
        return response()->json([
            'success' => true,
            'data' => $pendingTransaction, $customers, $pendingCart
        ]); //terkahir kita menggunakan format JSON dengab mengitimkan balik data transaksasi yang dibuat
    }
